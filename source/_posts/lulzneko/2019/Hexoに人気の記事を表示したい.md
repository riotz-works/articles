...
JAMStack にすることでパフォーマンスが良く、安全性を高めたサイトを作ることができます。詳しくは本ブログの記事 [JAMStack、それは ハイパフォーマンスなウェブフロントを実現するアーキテクチャ](https://riotz.works/articles/2019/01/23/jamstack-an-architecture-to-realize-fine-web-front/) をご参照いただければと思います。
...
そして形態素解析も備えていて、記事本文を分析して「関連する記事」のリストも作ってくれます。また「人気度」「関連度」を組み合わせることもできミックスしてどちらにウェイトを置いてリストを作るかという制御も可能です。
...
ローカルサーバーを使い動作確認を行うとウィジェットが表示されますが、記事のタイトルが大きいように感じます。これは hexo-related-popular-posts が生成するリストに `<h3>` タグが入っており若干大きく表示されているというのがあります。スタイルを適用してそろえるようにします。
...
[hexo-related-popular-posts](https://github.com/tea3/hexo-related-popular-posts)、Google Analytics を使うという着眼点と、それを実現する実装力、素晴らしいです！！
